...
let currentRadius = 200000; // Raio inicial em metros (200 km)
...
function handleMapDoubleClick(e) {
    ...
    let outerRadius, middleRadius, innerRadius;
    if (currentRadius === 200000) {
        outerRadius = 200000; // 200 km
        middleRadius = 133333; // ~133.333 km
        innerRadius = 66666; // ~66.666 km
    } else {
        outerRadius = 1300000; // 1.300 km
        middleRadius = (133333 / 200000) * 1300000; // Proporcional: ~866.665 km
        innerRadius = (66666 / 200000) * 1300000; // Proporcional: ~433.329 km
    }
...
function toggleRadius() {
    currentRadius = currentRadius === 200000 ? 1300000 : 200000;
    document.getElementById('toggleRadius').textContent = currentRadius === 200000 ? 'Raio 1' : 'Raio 2';

    // Atualizar círculos se houver um marcador
    if (clickMarker && markerCoords) {
        ...
        if (currentRadius === 200000) {
            ...
            outerRadius = 1300000;
            middleRadius = (133333 / 200000) * 1300000;
            innerRadius = (66666 / 200000) * 1300000;
        }
    ...
    for (let i = 0; i <= steps; i++) {
        ...
        if (positionAndVelocity.position) {
            ...
            if (latitudeRad < -Math.PI / 2 || latitudeRad > Math.PI / 2) {
                console.warn('Latitude inválida no cálculo do caminho orbital. Pulando ponto.');
                continue;
            }
            const latitude = satellite.degreesLat(latitudeRad);
...
function calculatePasses(satrec, centerLat, centerLng, startTime, days = 15) {
    ...
    for (let time = new Date(startTime); time <= endTime; time.setSeconds(time.getSeconds() + stepSeconds)) {
        ...
        if (positionAndVelocity.position) {
            ...
            const lat = satellite.degreesLat(latitudeRad);
            ...
            if (!isInside && wasInside) {
                let elevation;
                const innerRadius = (66666 / 200000) * currentRadius;
                const middleRadius = (133333 / 200000) * currentRadius;
                if (minDistance <= innerRadius) {
                    elevation = 'Alta';
                } else if (minDistance <= middleRadius) {
                    elevation = 'Mediana';
                } else {
                    elevation = 'Baixa';
                }
...
function generatePassPredictions() {
    ...
    passes.forEach(pass => {
        ...
        const encodedRadius = encodeURIComponent(currentRadius);

        html += `
            <tr>
                <td>${passTimeFormatted}</td>
                <td>${elevation}</td>
                <td><a href="#" onclick="simulatePass('${encodedTLE}', '${encodedSatName}', '${encodeURIComponent(passTimeLocal)}', '${encodedLat}', '${encodedLng}', '${encodedTimezone}', '${encodedRadius}'); return false;">Ver no Mapa</a></td>
            </tr>
        `;
    });
...
function simulatePass(encodedTLE, encodedSatName, passTimeLocal, encodedLat, encodedLng, encodedTimezone, encodedRadius) {
    // Rolar para o topo da página
    scrollToTop();

    if (encodedRadius) {
        currentRadius = parseInt(decodeURIComponent(encodedRadius));
        document.getElementById('toggleRadius').textContent = currentRadius === 200000 ? 'Raio 1' : 'Raio 2';
    }
...
function startTracking() {
    ...
    const meanMotionDerivative = parseFloat(tleLines[0].substring(33, 43));
    if (meanMotionDerivative > 0.01) {
        alert('Aviso: Este satélite tem uma alta taxa de decaimento orbital. A propagação pode ser imprecisa devido à possível reentrada.');
    }
    ...
    try {
        ...
        if (!satrec || isNaN(satrec.inclo) || satrec.inclo === undefined) {
            alert('Erro: Não foi possível inicializar os dados orbitais. Verifique a validade da TLE.');
            return;
        }

        const inclinationDegrees = satrec.inclo * (180 / Math.PI);
        if (isNaN(inclinationDegrees)) {
            alert('Erro: Inclinação orbital inválida. A TLE pode estar corrompida.');
            return;
        }
        ...
        function updatePosition() {
            ...
            try {
                ...
                if (positionAndVelocity.position && positionAndVelocity.velocity) {
                    ...
                    const latitudeRad = positionGd.latitude;
                    if (latitudeRad < -Math.PI / 2 || latitudeRad > Math.PI / 2) {
                        console.warn('Latitude fora do intervalo válido. Possível erro na propagação ou reentrada do satélite.');
                        return;
                    }
                    ...
                    if (altitude < 100) {
                        console.warn(`Altitude muito baixa (${altitude.toFixed(2)} km). O satélite pode ter reentrado.`);
                        alert(`Aviso: A altitude do satélite ${satNameGlobal} é muito baixa (${altitude.toFixed(2)} km). Ele pode ter reentrado na atmosfera.`);
                        clearInterval(interval);
                        return;
                    }
                    ...
                    alert(`Erro ao propagar a órbita do satélite ${satNameGlobal}. Verifique a TLE ou tente outra data.`);
                    clearInterval(interval);
                }
            } catch (error) {
                console.error('Erro durante a propagação ou conversão de coordenadas:', error);
                alert(`Erro ao rastrear o satélite ${satNameGlobal}. Possível problema com a TLE ou reentrada.`);
                clearInterval(interval);
            }
        }
        ...
        console.error('Erro ao inicializar satrec:', error);
        alert('Erro: Não foi possível processar a TLE. Verifique se ela está correta e tente novamente.');
    }
}
...
function loadFromUrlParams() {
    ...
    if (radius) {
        currentRadius = parseInt(decodeURIComponent(radius));
        document.getElementById('toggleRadius').textContent = currentRadius === 200000 ? 'Raio 1' : 'Raio 2';
    }
